...
// when using mongodb replication, i.e.
// when running a mongodb cluster, you can define your replication set here;
// when you are not using replication (most of the users), just set it to `null` (default).
...
// please note that bittrex API is limited, you cannot use backfills or sims
...
// However margin trading may not work...read the API documentation.
...
// (WARNING : sim won't work properly if you set this value to 100)
c.buy_pct = 99;
...
// time to leave an order open, default to 1 day
// (this feature is not supported on all exchanges, currently: GDAX)
c.cancel_after = 'day';
...
// xmpp config
c.notifiers.xmpp = {};
// false xmpp disabled; true xmpp enabled (credentials should be correct)
c.notifiers.xmpp.on = false;
...
// false pushbullets disabled; true pushbullets enabled (key should be correct)
c.notifiers.pushbullet.on = false;
...
// discord configs
c.notifiers.discord = {};
// false discord disabled; true discord enabled (key should be correct)
c.notifiers.discord.on = false;
...
// false textbelt disabled; true textbelt enabled (key should be correct)
c.notifiers.textbelt.on = false;
